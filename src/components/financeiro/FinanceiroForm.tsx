...
export default function FinanceiroForm({ initial, onSave, onCancel }: FinanceiroFormProps) {
  const [conta, setConta] = useState<ContaFinanceira>(initial || { descricao: '', valor: 0, tipo: 'receita', dataVencimento: '' });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input name="descricao" value={conta.descricao} onChange={handleChange} required className="input" placeholder="Digite a descrição" />
      </div>
      <div>
        <label className="block font-medium">Valor</label>
        <input type="number" name="valor" value={conta.valor} onChange={handleChange} required className="input" min={0} step={0.01} placeholder="Digite o valor" />
      </div>
      <div>
        <label className="block font-medium">Tipo</label>
        <select name="tipo" value={conta.tipo} onChange={handleChange} className="input" title="Selecione o tipo">
      ...
      <div>
        ...
        <input type="date" name="dataVencimento" value={conta.dataVencimento} onChange={handleChange} required className="input" placeholder="Selecione a data de vencimento" />
      </div>
